...
from tkinter import Tk, Label, Button, filedialog, messagebox, Frame, IntVar, StringVar
from tkinter.ttk import Progressbar, Separator, OptionMenu
...
color_scheme = StringVar()  # 用于保存用户选择的颜色方案
color_scheme.set("viridis")  # 默认配色方案

# 新增全局变量初始化
color_choice = "black"  # 默认颜色
...
def create_wordcloud(input_path, output_path, stopwords_path=None, colormap="viridis"):
    """生成词云和词频统计"""
...
def start_processing():
    ...
    # 使用用户选择的颜色方案
    create_wordcloud(input_path, output_path, stopwords_file, color_scheme.get())
...
# 初始化 color_scheme 变量（必须在 root 初始化后）
color_scheme = StringVar(root)  # 指定 master 为 root
color_scheme.set("viridis")  # 默认配色方案
...
# 配色选择
color_label = Label(root, text="选择词云配色方案：", font=("Arial", 10), fg="black")
color_label.pack(pady=5)

color_options = ["viridis", "plasma", "coolwarm", "spring", "summer", "autumn", "winter"]
color_menu = OptionMenu(root, color_scheme, *color_options)
color_menu.pack(pady=5)

# 开始按钮
start_button = Button(root, text="生成词云", command=start_processing, bg="green", fg="white", font=("Arial", 12), width=20)
